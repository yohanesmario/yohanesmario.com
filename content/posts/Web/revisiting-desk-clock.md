...
To test this, you can visit [https://yohanesmario.com/projects/time?tz=Asia/Makassar](https://yohanesmario.com/projects/time?tz=Asia/Makassar). This will not auto detect your timezone, as that's impossible to do on the kindle browser, so you'll need to provide your timezone using the `tz` query param. We also need to enable display orientation changes, so I just add an on click listener to rotate the entire page 90 degrees on each click.
